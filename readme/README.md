...
- [日常使用](#日常使用)
  ...
  - [功能四：匯出庫存報表](#功能四匯出庫存報表)
  - [功能五：Agent mode](#功能五agent-mode-ai-助理)
- [常見問題](#常見問題)
...
**適用情境：** 需要將 Ragic 倉庫庫存匯出成 Excel 報表（報價單或庫存總覽）。

**操作步驟：**

1. 選「匯出庫存報表（Excel）」
2. 選擇倉庫
3. 用**空白鍵**勾選模板（報價單 / 庫存總覽），按 **Enter** 確認
4. 確認執行後，檔案自動儲存至 `exports/` 資料夾

---

### 功能五：Agent mode（AI 助理）

**適用情境：** 用自然語言查詢庫存、分析銷售、取得補貨建議。

> 需要 Anthropic API Key，第一次使用時程式會提示輸入並自動儲存。
...
|------|------|
| `退出` | 離開 Agent mode |
| `重設 key` 或 `reset key` | 更換 Anthropic API Key |
