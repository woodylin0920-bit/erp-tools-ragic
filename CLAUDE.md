...
- **批次發樣／開免費單**：新品發樣用。選單別(樣品申請/公關品/活動贈品)+組合(中文/英文/代號搜尋商品建範本，存 `app/sample_combos.json`)+客戶名單(從 20004 勾選，存 `app/sample_customer_lists.json`，兩 json 皆 gitignore 不上傳)→ 批次 POST 到 20001，單價全0、狀態未出貨。吃 dry-run+確認關卡。CLI handler `run_sample_orders`；純邏輯抽在 `app/sample_core.py`（`create_sample_orders` commit=False 只回 payload 不寫入），CLI 與 GUI 共用。
- **桌面 GUI（開發中）**：`app/gui.py`（customtkinter），階段 2 雛形先做「批次發樣」一個畫面，走 `sample_core`。啟動 `python3 app/gui.py` 或 `start-gui.command`/`start-gui.bat`。需 Python 含 tkinter（macOS: `brew install python-tk`）。設計依據在 `design/prototype-desktop.html`。其餘功能逐步搬入；CLI（`ragic_upload.py`）仍是現役完整入口
