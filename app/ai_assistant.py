...
def run_agent_mode():
    ...
    console.print(Text("  Agent mode — 自然語言 AI 助理", style="bold #C5A059"))
    console.print(Rule(style="#C5A059"))
    console.print()
    console.print("[#B0A898]你可以這樣問：[/#B0A898]")
    console.print()
    console.print("  [#C5A059]查詢庫存[/#C5A059]")
    console.print("    [dim]「TW01 現在還有多少 BBB042？」[/dim]")
    console.print("    [dim]「幫我查所有倉庫的庫存狀況」[/dim]")
    console.print()
    console.print("  [#C5A059]銷售分析[/#C5A059]")
    console.print("    [dim]「1～3 月銷售前五名 SKU 是哪些？」[/dim]")
    console.print("    [dim]「上個月哪些客戶沒有下單？」[/dim]")
    console.print("    [dim]「新光三越最近一次下單是什麼時候？」[/dim]")
    console.print()
    console.print("  [#C5A059]採購建議[/#C5A059]")
    console.print("    [dim]「根據過去三個月銷量，BBB042 應該補多少貨？」[/dim]")
    console.print("    [dim]「哪些 SKU 庫存不足，需要補貨？」[/dim]")
    console.print()
    console.print("  [#C5A059]匯出報表[/#C5A059]")
    console.print("    [dim]「把本月未出貨的訂單整理成 Excel 存到桌面」[/dim]")
    console.print("    [dim]「幫我匯出 TW01 庫存不足 10 件的商品清單」[/dim]")
    console.print()
